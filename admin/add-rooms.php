<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $sql="SELECT room_no FROM rooms where room_no=? AND tenant_id=? AND hostel_id <=> ?";
        $stmt1 = $mysqli->prepare($sql);
        $stmt1->bind_param('iii',$roomno, $tenantId, $hostel_id);
        $stmt1->execute();
        $stmt1->store_result(); 
        $row_cnt=$stmt1->num_rows;;
        if($row_cnt>0){
            include_once('../includes/toast-helper.php');
            setToast('warning', 'Room already exist in this hostel!');
        } else {
            $query="INSERT into  rooms (seater,room_no,fees,tenant_id,hostel_id) values(?,?,?,?,?)";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('iiiii',$seater,$roomno,$fees,$tenantId,$hostel_id);
            if($stmt->execute()){
                ...
                <form method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-6 col-lg-4">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Select Hostel</h4>
                                        <div class="form-group mb-4">
                                            <select class="custom-select mr-sm-2" id="hostel" name="hostel">
                                                ...
                                                while($h_row = $h_res->fetch_object()){
                                                    echo "<option value=\"{$h_row->id}\">" . htmlentities($h_row->name) . "</option>";
                                                }
                                                ?>
                                            </select>
                                        </div>
