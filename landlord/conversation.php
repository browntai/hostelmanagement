<?php
    ...
    $isAdminConvo = ($recipientRole === 'admin');

    // Mark as read
    if($recipient_id){
        if($isAdminConvo){
            $update_q = "UPDATE messages SET is_read=1 WHERE sender_id=? AND receiver_id=? AND tenant_id IS NULL AND is_read=0";
            $stmt_up = $mysqli->prepare($update_q);
            $stmt_up->bind_param('ii', $recipient_id, $adminId);
        } else {
            $update_q = "UPDATE messages SET is_read=1 WHERE sender_id=? AND receiver_id=? AND tenant_id=? AND is_read=0";
            $stmt_up = $mysqli->prepare($update_q);
            $stmt_up->bind_param('iii', $recipient_id, $adminId, $tenantId);
        }
        $stmt_up->execute();
    }
    ...
    if(isset($_POST['send_message'])){
        ...
        // Use NULL tenant_id for Super Admin conversations
        $target_tenant_id = ($target_role === 'admin') ? NULL : $tenantId;
        ...
        if($stmt->execute()){
            ...
            sendNotification($target_recipient_id, 'New Message from Landlord', "You have a new message from " . ($_SESSION['full_name']), $adminId);
                                ...
                                <form method="POST">
                                    <div class="form-group">
                                        ...
                                        <select name="client_id" id="client_id" class="form-control" required>
                                            ...
                                            <?php
                                            // Landlords see their clients AND the Super Admin
                                            // 1. Fetch Super Admin
                                            $sa_q = "SELECT id, full_name, email FROM users WHERE role='admin' AND tenant_id IS NULL LIMIT 1";
                                            $sa_res = $mysqli->query($sa_q);
                                            if($sa_row = $sa_res->fetch_object()){
                                                echo "<option value='{$sa_row->id}'>{$sa_row->full_name} ({$sa_row->email}) - [Super Admin]</option>";
                                            }

                                            // 2. Fetch clients who have booked their hostels
                    ...
                    <div class="col-md-12">
                        <div class="card chat-container">
                            <div class="chat-header">
                                ...
                                <span class="text-muted small"><?php echo $isAdminConvo ? 'Super Admin' : 'Client'; ?></span>
                                </div>
                            </div>
                            ...
                            <div class="chat-messages" id="chatbox">
                                <?php
                                if($isAdminConvo){
                                    // Super Admin conversation: tenant_id IS NULL
                                    ...
                                    // Client conversation: use tenant_id
                                    $query = "SELECT * FROM messages WHERE 
                                              ((sender_id=? AND receiver_id=?) OR 
                                               (sender_id=? AND receiver_id=?))
                                              AND tenant_id=?
                                              ORDER BY created_at ASC";
                                    $stmt = $mysqli->prepare($query);
                                    $stmt->bind_param('iiiii', $recipient_id, $adminId, $adminId, $recipient_id, $tenantId);
                                }
