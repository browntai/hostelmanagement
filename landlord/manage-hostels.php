<?php
    ...
    if(isset($_GET['del'])){
        ...
        // Delete hostel (CASCADE will handle related records)
        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-md-6 col-lg-8">
                        <div class="customize-input">
                            <div class="input-group">
                                ...
                                <input class="form-control border-0 shadow-sm" type="search" id="hostelSearch" placeholder="Search my hostels by name or location..." aria-label="Search">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row" id="hostelGrid">
                    <?php
                    $query = "SELECT * FROM hostels WHERE tenant_id = ? ORDER BY created_at DESC";
                    $stmt = $mysqli->prepare($query);
                    ...
                    <?php
                        ...
                        while($row = $res->fetch_object()):
                            ...
                            // 1b. Get Available Room Count
                            $avail_count_query = "SELECT COUNT(*) as count FROM rooms WHERE hostel_id = ? AND status = 'available'";
                            $stmt_avail = $mysqli->prepare($avail_count_query);
                            $stmt_avail->bind_param('i', $row->id);
                            $stmt_avail->execute();
                            $avail_result = $stmt_avail->get_result();
                            $avail_data = $avail_result->fetch_object();
                            $available_room_count = $avail_data ? $avail_data->count : 0;
                            $stmt_avail->close();
                                ...
                                <div class="bg-light rounded p-3 mb-3">
                                    <div class="row text-center">
                                        <div class="col-6 border-right">
                                            <small class="text-muted d-block text-uppercase small">Availability</small>
                                            <h5 class="mb-0 font-weight-bold <?php echo ($available_room_count > 0) ? 'text-success' : 'text-danger'; ?>">
                                                <?php echo $available_room_count; ?> <small class="text-muted">/ <?php echo $actual_room_count; ?></small>
                                            </h5>
                                        </div>
                                        <div class="col-6">
